...
hole::hole(float posX, float posY, int size, sf::Color colour, b2World * world, Conversion &convert)
{
	...
	m_b2Shape1.SetAsBox(length / 3, length / 2, b2Vec2(0 - length / 3, 0), 0);
	...
	m_b2Shape2.SetAsBox(length / 3, length / 20, b2Vec2(0, length / 40), 0);
	...
	m_b2Shape3.SetAsBox(length / 3, length / 2, b2Vec2(length / 3, 0), 0);
...
{
	if (m_sfRectShape.getGlobalBounds().contains(
		m_convert->box2DXToCanvas(shape->GetBodyPosition().x), 
		m_convert->box2DXToCanvas(shape->GetBodyPosition().y))
		)
		return true;
	return false;
}
